...
void ST77922::Fill_Colors(uint16_t sx, uint16_t sy, uint16_t w, uint16_t h, uint16_t* color)
{
    ...
    uint16_t row, col;
    uint16_t phys_sx, phys_sy, phys_w, phys_h;
    ...
    if(((w < 1) || (w > width)) || ((h < 1) || (h > height)))
        return;
	/* rotation 1/3：面板 MADCTL 仍为竖屏，这里做软件旋转。
	 * color 必须是紧凑的 w*h 缓冲（LVGL 局部刷新 / Sprite 全屏均可）。
	 * 旧实现按绝对坐标索引，局部刷新会越界导致复位。 */
	if((rotation == 1)||(rotation == 3))
    {
        ...
        if(rotation == 1)
        {
            /* logical(lx,ly) -> physical(LCD_WIDTH-1-ly, lx) */
            phys_sx = LCD_WIDTH - sy - h;
            phys_sy = sx;
            phys_w = h;
            phys_h = w;
            for(row = 0; row < h; row++)
            {
                for(col = 0; col < w; col++)
                {
                    cbuf[col * phys_w + (phys_w - 1 - row)] = color[row * w + col];
                }
            }
        }
        else
        {
            /* logical(lx,ly) -> physical(ly, LCD_HEIGHT-1-lx) */
            phys_sx = sy;
            phys_sy = LCD_HEIGHT - sx - w;
            phys_w = h;
            phys_h = w;
            for(row = 0; row < h; row++)
            {
                for(col = 0; col < w; col++)
                {
                    cbuf[(w - 1 - col) * phys_w + row] = color[row * w + col];
                }
            }
        }
		tx_buf = cbuf;
